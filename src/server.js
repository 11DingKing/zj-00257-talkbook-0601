...
app.get("/", (req, res) => {
  res.json({
    ...
    docs: {
      ...
      承诺跟踪: {
        "GET /api/promises":
          "承诺列表（支持 vendorId, talkId, status 过滤，含核验摘要信息）",
        "GET /api/promises/:id":
          "承诺详情（含材料提交记录、历次核验记录、完整时间线）",
        "POST /api/promises/:id/evidence":
          "厂商提交整改佐证材料（自动变更状态为「待核验」）",
        "POST /api/promises/:id/verify":
          "监管核验（通过→置为按期完成；不通过→退回整改中，附原因）",
        "PUT /api/promises/:id/status":
          "更新承诺状态（状态流转，含「待核验」中间状态）",
      },
      升级处置: {
        "GET /api/escalations":
          "升级处置列表（支持 vendorId, type, handled 过滤）",
        "PUT /api/escalations/:id/handle": "标记处置已处理",
      },
      统计分析: {
        "GET /api/stats/overview":
          "总览统计（含一次性核验通过率、待核验数、近30天核验数据）",
        "GET /api/stats/by-vendor":
          "按厂商统计（约谈次数/按期完成率/一次性核验通过率/升级处置数）",
        "GET /api/stats/by-reason":
          "按事由统计（约谈次数/按期完成率/一次性核验通过率/升级处置数）",
      },
    },
  });
});
  ...
  () => {
    try {
      ...
      if (newEscalations.length > 0) {
        console.log(
          `[自动检查] ${new Date().toLocaleString()} 触发 ${newEscalations.length} 条逾期升级处置`,
        );
      }
    } catch (e) {
